...
######## fun_inter_ticks() #### define coordinates of secondary ticks for log scales


fun_inter_ticks <- function(lim, log = "log10", lib.path = NULL){
# AIM
# define coordinates of secondary ticks for log scales
# ARGUMENTS
# lim: vector of 2 numbers indicating the limit range of the axis. Order of the 2 values matters (for inverted axis). Must be already log transformed values. Thus, negative or zero values are allowed
# log: either "log2" (values in the lim argument are log2 transformed) or "log10" (values in the lim argument are log10 transformed)
# lib.path: absolute path of the required packages, if not in the default folders
# REQUIRED PACKAGES
# none
# REQUIRED FUNCTIONS FROM CUTE_LITTLE_R_FUNCTION
...
# $values: the corresponding values associated to each coordinate (2^$values or 10^$values form the labels of the axis)
# EXAMPLES
# log2
# fun_inter_ticks(lim = c(-4,4), log = "log2", lib.path = NULL)
# log10
# fun_inter_ticks(lim = c(-2,3), log = "log10", lib.path = NULL)
# DEBUGGING
# lim = c(2, 3.101) ; log = "no" ; lib.path = NULL # for function debugging
# function name
function.name <- paste0(as.list(match.call(expand.dots=FALSE))[[1]], "()")
# end function name
# required function checking
...
# arg with no default values
if(any(missing(lim))){
tempo.cat <- paste0("\n\n================\n\nERROR IN ", function.name, ": ARGUMENT lim HAS NO DEFAULT VALUE AND REQUIRE ONE\n\n================\n\n")
stop(tempo.cat, call. = FALSE)
}
# end arg with no default values
# using fun_check()
...
tempo <- fun_check(data = log, options = c("log2", "log10"), length = 1, fun.name = function.name) ; eval(ee)
if( ! is.null(lib.path)){
tempo <- fun_check(data = lib.path, class = "vector", mode = "character", fun.name = function.name) ; eval(ee)
if(tempo$problem == FALSE){
if( ! all(dir.exists(lib.path))){ # separation to avoid the problem of tempo$problem == FALSE and lib.path == NA
...
# second round of checking and data preparation
# dealing with NA
if(any(is.na(lim)) | any(is.na(log)) | any(is.na(lib.path))){
...
tempo.cat <- paste0("\n\n================\n\nERROR IN ", function.name, ": THESE ARGUMENTS lim AND log CANNOT BE NULL\n\n================\n\n")
stop(tempo.cat, call. = FALSE)
}
# end dealing with NULL
# end second round of checking and data preparation
# main code
lim.order <- order(lim) # to deal with inverse axis
ini.scipen <- options()$scipen
options(scipen = -1000) # force scientific format
...
options(scipen = ini.scipen) # restore the initial scientific penalty
tick.pos <- unique(as.vector(outer(2:10, ifelse(log == "log2", 2, 10)^((power10.exp[1] - ifelse(diff(lim.order) > 0, 1, -1)):(power10.exp[2] + ifelse(diff(lim.order) > 0, 1, -1)))))) # use log10(2:10) even if log2: it is to get log values between 0 and 1
tick.pos <- sort(tick.pos, decreasing = ifelse(diff(lim.order) > 0, FALSE, TRUE))
if(log == "log2"){
...
tempo.cat <- paste0("\n\n============\n\nINTERNAL CODE ERROR IN ", function.name, ": NA or Inf GENERATED FOR THE INTER TICK POSITIONS: ", paste(tick.pos, collpase = " "), "\n\n============\n\n")
stop(tempo.cat)
}
...
tempo.tick.pos <- tempo$coordinates
...
# title: character string of the graph title
# title.text.size: numeric value of the title size (in points)
# legend.show: logical. Show legend? Not considered if categ argument is NULL, because this already generate no legend
# legend.width: single proportion (between 0 and 1) indicating the relative width of the legend sector (on the right of the plot) relative to the width of the plot. Value 1 means that the window device width is split in 2, half for the plot and half for the legend. Write NULL to inactivate the legend sector. In such case, ggplot2 will manage the room required for the legend display, meaning that the width of the plotting region can vary between graphs, depending on the text in the legend
...
minor.tick.pos <- minor.tick.pos[minor.tick.pos >= min(x.lim) & minor.tick.pos <= max(x.lim)]
...
x.second.tick.values <- minor.tick.pos
coord.names <- c(coord.names, "x.second.tick.positions")
}
}
